...
Pack&Playは、VTuber/ストリーマー向けのカードパック販売プラットフォームです。ストリーマーが作ったカード入りの"パック"を視聴者が購入して開封し、当たったカードを使うと配信画面でエフェクトが出たり、ストリーマーにお題・リクエストが届くサービスです。スパチャのように応援しつつ、"参加して一緒に遊ぶ"体験を作れます。
...
1. **ソーシャルログイン** (`index.html` - エントリーポイント)
   - 視聴者/ストリーマーロール選択
   - YouTubeでログイン（視聴者・ストリーマー共通）
   - Twitchでログイン（視聴者・ストリーマー共通）
   - Xでログイン（視聴者のみ）
...
- **ストリーマーとしてログイン**: ダッシュボード、カード管理、承認キューへアクセス可能
  - YouTube、Twitchのいずれかでログイン（Xは非対応）

### 視聴者向け機能

1. **ストリーマー発見** (`discover.html`)
   - ストリーマーの一覧表示
   - 配信中バッジ表示
   - 検索機能

2. **ストリーマーページ** (`creator/tanaka.html`)
   - パック一覧
   - 購入ボタン
...
- `creatorCards`: ストリーマーのカードライブラリ
- `creatorPacks`: ストリーマーのパック
- `redemptions`: 承認待ちキュー
...
```javascript
{
  ...
  name: '表示名',
  creatorSlug: 'tanaka', // ストリーマーの場合のみ
...
`js/mock-data.js` を編集して、ストリーマーやパック、カードのデータを変更できます。
...
4. **ストリーマー発見**: `discover.html` でストリーマーを探す
5. **パック購入**: ストリーマーページでパックを選択 → 詳細確認 → 購入
...
1. **ランディングページ**: `index.html` を開く
2. **即ログイン（推奨）**: 「🎬 ストリーマーとして始める」をクリック → YouTubeで自動ログイン
